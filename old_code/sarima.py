...
class sarimaClass():
    ...
    def fit_arima(self, name):
        ...
        start = time.time()
        #Model estimation
        arima_model = pm.auto_arima(s, start_p=2, d=0, start_q=2, max_p=7, max_q=7,
                        srtart_P=1, start_Q = 1, max_P = 4, max_Q = 4, m = 7, seasonal=True, 
                        stepwise=True, suppress_warnings=True, error_action='ignore', maxiter=200)
    ...
    def predict(self, name):
        ...
        for value in test:#.loc[start_value:]:
            ...
            model.update(value, maxiter=5) # Update time series with actual value for next prediction
